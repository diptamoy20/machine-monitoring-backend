...
from trcker_utils import MachineTracker
...
def run_on_video(video_path, model, roi_manager, smoother):
    ...
    recorder = ClipRecorder(config.DETECTION_DIR, config.RECORD_SECONDS)
    machine_tracker = MachineTracker()
    ...
    while cap.isOpened():
        ...
        roi_to_machine_id = machine_tracker.update(rois)
...
def main():
    ...
    roi_manager = ROIManager(config.ROI_CONFIG_PATH)
